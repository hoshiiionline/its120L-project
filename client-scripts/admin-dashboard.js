function reloadBookings() {
  Promise.all([
    fetch("../client-api/pendingBooking.php").then((res) => res.json()),
    fetch("../client-api/approvedBooking.php").then((res) => res.json()),
  ])
            ...
            <td>
                ...
                <select class="status-select" data-id="${room.bookingID}">
                    ...
                    <option value="APPROVED" ${
                      room.status === "APPROVED" ? "selected" : ""
                    }>Approved</option>
                    <option value="CANCELLED" ${
                      room.status === "CANCELLED" ? "selected" : ""
                    }>Cancelled</option>
                </select>
            </td>
        `;
